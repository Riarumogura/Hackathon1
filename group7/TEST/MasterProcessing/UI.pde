...
void setup() {
  ...
  try {
    ...
    // ★ このマシンでの計測用に実ポート名を直接指定（Master Arduino）
    myPort = new Serial(this, "/dev/cu.usbmodem21401", 115200);
    myPort.bufferUntil('\n'); // 改行コードが来るまでデータを溜める
  ...
  if (LATENCY_TEST) {
    tickLog = createWriter("latency_log_master_ticks.csv");
    tickLog.println("tick_id,send_log_recv_time_ms");
  }
...
void serialEvent(Serial myPort) {
  ...
  if (inString != null) {
    ...
    // MA→SP遅延計測用：MasterArduinoが出すTick送信ログをそのまま利用してタイムスタンプを記録
    // 例: "[BPM Tick] Sent: 120  tick: 5"
    else if (LATENCY_TEST && inString.startsWith("[BPM Tick] Sent:")) {
      long now = System.currentTimeMillis();
      String[] tok = split(inString, "tick:");
      if (tok.length == 2) {
        int tickId = int(trim(tok[1]));
        tickLog.println(tickId + "," + now);
        tickLog.flush();
      }
    }
  }
...
void completeCurrentRun(String message) {
  isPlaying = false;
  // 演奏終了後に演奏順・オクターブを変更して再送信できるよう，初回送信フラグを戻す
  // （次回の「設定確定・送信」は3桁BPM更新ではなく8桁の完全な設定パケットになる）
