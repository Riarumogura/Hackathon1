...
#define LATENCY_TEST          // MA→SP遅延計測時のみ有効化する（TEST専用，本番では使わない）
...
void loop() {
  if (dataReceived) {
    ...
    if (input.length() == 8) {
      ...
      // パケット内の演奏順フィールドは「ピアノ・フルート・木琴」の順（index 2,3,4）。
      // INST_IDの番号（ピアノ=1,木琴=2,フルート=3）とは並びが異なるため，
      // INST_IDから直接 index = INST_ID+1 を計算すると木琴とフルートの値が入れ替わってしまう。
      // そのため楽器ごとに正しいフィールド位置を明示的に指定する。
      int orderFieldIndex;
...
#ifdef LATENCY_TEST
            // MasterArduinoが "[BPM Tick] Sent: ... tick: N" でログするNと一致する
            // （tickCountはconfig送信を1として数えるため，常に tickCount = i2cReceiveCount + 1）
            int tickId = i2cReceiveCount + 1;
#endif
