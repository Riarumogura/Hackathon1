/**
 * ============================================================
 * Flute Synthesis v4.5 — 音色チューニング版
 * ============================================================
 *
 ...
 *
 * v4 → v4.5 の変更点:
 *   - PITCH_SHIFT 定数追加: フォールバック再生を +12 半音（C5-C6）に
 *   - getRegister 閾値を実フルートに合わせ 554Hz / 1109Hz に変更
 *   - AMPS（倍音バランス）を基音支配型に修正（クラリネット臭さ除去）
 *   - ATK 0.4秒台 → 25-80ms（八分音符が立ち上がる前に終わる問題を解消）
 *   - DEC 短縮 / SUS 0.92-0.95 に上げ / REL 滑らか化（envelope の「跳ね→減衰」感を抑制）
 *   - 低音は暖かみ / 高音はクリアという実フルート特性を維持
 *
 * フォールバック動作:
 *   - 1〜8 キー: 単音再生（PITCH_SHIFT 適用後の音域）
 *   - p キー: 森のくまさん全曲（Serial 無しでの動作確認）
 */
...
void serialEvent(Serial p) {
  ...
  try {
    int midi       = int(parts[0]);
    int durationMs = int(parts[1]);
